...
$sql = "UPDATE customers  set  intentos = ? WHERE id = ?";
			$q = $pdo->prepare($sql);
			$q->execute(array($intentos,$id));
			Database::disconnect();
			header("Location: ../devolucion.php");
		}
	} else {
...
<head>
    <meta charset="utf-8">
 <title>INSTITUTO POSRAL DOMINICANO! | SEDE</title>
					...
					<!--Comienzo-->
					  ...
					  <!--Tercero-->
					   ...
					   <div class="control-group <?php echo !empty($monitoresError)?'error':'';?>">
					    ...
					    <div class="controls">
					      	<input name="monitores" type="text" disabled placeholder="monitores" value="<?php echo !empty($monitores)?$monitores:'';?>">
					      	<?php if (!empty($monitoresError)): ?>
					      		<span class="help-inline"><?php echo $monitoresError;?></span>
					      	<?php endif; ?>
					    </div>
					  </div>
                      <!--CARTEROS-->
					   <div class="control-group <?php echo !empty($carteroError)?'error':'';?>">
					    <label class="control-label">Cartero</label>
					    <div class="controls">
					      	<input name="cartero" type="text" disabled placeholder="cartero" value="<?php echo !empty($cartero)?$cartero:'';?>">
					      	<?php if (!empty($carteroError)): ?>
					      		<span class="help-inline"><?php echo $carteroError;?></span>
					      	<?php endif; ?>
					    </div>
					  </div>
					  <!--OFICIALES-->
					   <div class="control-group <?php echo !empty($oficialesError)?'error':'';?>">
					    ...
					    <div class="controls">
					      	<input name="oficiales" type="text" disabled placeholder="oficiales" value="<?php echo !empty($oficiales)?$oficiales:'';?>">
					      	<?php if (!empty($oficialesError)): ?>
					      		<span class="help-inline"><?php echo $oficialesError;?></span>
					      	<?php endif; ?>
					    </div>
					  </div>
					  <!--DESTINATARIO-->
					  ...
					  <!--Numero Diez-->
                       <div class="control-group <?php echo !empty($despacho_servicioError)?'error':'';?>">
					    ...
					    <div class="controls">
					      	<input name="despacho_servicio" type="text" disabled placeholder="# despacho" value="<?php echo !empty($despacho_servicio)?$despacho_servicio:'';?>">
                       ...
                       <!--Textarea motivo-->
                      <div class="control-group <?php echo !empty($intentosError)?'error':'';?>">
					   <label class="control-label">Intentos de Entrega</label>
					   <div class="controls">
					   <textarea name="intentos" class="form-control" rows="5" ><?php echo !empty($intentos)?$intentos:'';?></textarea>
					      	<?php if (!empty($intentosError)): ?>
					      		<span class="help-inline"><?php echo $intentosError;?></span>
					      	<?php endif; ?>
