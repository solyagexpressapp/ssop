...
$sql = "UPDATE customers  set ubicacion = ? WHERE id = ?";
			$q = $pdo->prepare($sql);
			$q->execute(array($ubicacion,$id));
			Database::disconnect();
			header("Location: ../devolucion.php");
		}
	} else {
    ...
    <div class="container">
    			...
    			<div class="span10 offset1">
					...
					<!--Comienzo-->
					      	...
					      	<?php
                               include '../conexion.php';
                                $sql = "SELECT * FROM monitores order by oficiales asc";
                                  $result = mysql_query($sql); ?>
                                     <select name="monitores" disabled>
					      	...
					      	<?php
                               include '../conexion.php';
                                $sql = "SELECT * FROM oficiales order by oficial asc";
                                  $result = mysql_query($sql); ?>
                                     <select name="oficiales" disabled>
					  ...
					  <!--Numero Once-->
					   <div class="control-group <?php echo !empty($ubicacionError)?'error':'';?>">
					    <label class="control-label">Ubicacion</label>
					    <div class="controls">
					      	<input name="ubicacion" type="text"  placeholder="Ubicacion" value="<?php echo !empty($ubicacion)?$ubicacion:'';?>">
