...
$result = mysqli_query($con,"SELECT numero_envio, estafeta.destino AS destino,fecha_entrada,destinatario,monitores,oficiales
FROM narisol_bladi.customers
    ...
    INNER JOIN narisol_bladi.division_regional 
        ...
        WHERE  servicio = 'MENSAJERIA'  AND DATE_FORMAT(fecha_entrada,'%Y-%m-%d') = DATE_FORMAT(SYSDATE(),'%Y-%m-%d')  AND  (customers.destino LIKE '%$destino1%') AND  (customers.monitores LIKE '%$oficial%')
 ORDER BY id DESC ");
...
echo "<td align='center'><span class='h'><strong>MENSAJERIA EXPRESA</strong></span><span class='d'><strong>". $row['fecha_entrada'] ."</strong></span></td>";
echo "</tr>";
echo "<tr>";
echo "<td><strong>DESTINATARIO:</strong></td>";
echo "<td colspan='3' align='center'>". $row['destinatario'] ."</td>";
echo "</tr>";
echo "<tr>";
...
<img width='250px'  src='barcodegen/test_1D.php?text=".$row['numero_envio']." ' alt='barcode'> </td>";
echo "</tr>";
echo "<tr>";
