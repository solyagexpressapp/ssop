<?php 
	...
	if ( !empty($_POST)) {
		...
		if ($valid) {
			...
			$sql = "UPDATE customers  set servicio = ?,destino =?,destinatario = ?,direccion = ?,telefono = ?,descripcion = ?,despacho_servicio =? WHERE id = ?";
			$q = $pdo->prepare($sql);
			$q->execute(array('MENSAJERIA',$destino,$destinatario,$direccion,$telefono,$descripcion,$despacho_servicio,$id));
			Database::disconnect();
