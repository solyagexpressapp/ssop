...
$fecha = date('Y-m-d');	
	...
	if ( !empty($_POST)) {
		...
		if ($valid) {
			...
			$sql = "UPDATE customers  set presinto_servicio = ?,destino = ?, fecha_entrada = ? ,despacho_servicio = ?, porque= ?, motivo = ? , condicion_general = 1  WHERE id = ?";
			$q = $pdo->prepare($sql);
			$q->execute(array($presinto_servicio,$destino,$fecha,$despacho_servicio,$porque,$devolucion,$id));
			Database::disconnect();
			header("Location: ../devolucion.php");
		}
	} else {
