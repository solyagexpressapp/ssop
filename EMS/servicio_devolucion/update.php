<?php 
	...
	if ( !empty($_POST)) {
		...
		if ($valid) {
			...
			$sql = "UPDATE customers  set servicio = ?,numero_envio = ?,destino =?,destinatario = ?,direccion = ?,peso_real = ?,telefono = ?,descripcion = ?,presinto_servicio = ?,despacho_servicio =?,colectora = NULL, despacho_general = NULL  WHERE id = ?";
			$q = $pdo->prepare($sql);
