<?php 
	...
	if ( !empty($_POST)) {
		...
		if ($valid) {
			...
			$sql = "UPDATE customers  set servicio = ?,numero_envio = ?,destino =?,destinatario = ?,direccion = ?,peso_real = ?,telefono = ?,descripcion = ?,presinto_servicio = ?,despacho_servicio =? WHERE id = ?";
			$q = $pdo->prepare($sql);
			$q->execute(array(EMS,$numero_envio,$destino,$destinatario,$direccion,$peso_real,$telefono,$descripcion,$presinto_servicio,$despacho_servicio,$id));
			Database::disconnect();
			header("Location: ../ems.php");
		}
	} else {
    ...
    <div class="container">
    			...
    			<div class="span10 offset1">
					...
					<!--Comienzo-->
					    ...
					    <div class="controls">
					      	<input name="servicio" type="text"  placeholder="fecha de entrada" disabled value="EMS">
					  ...
					  <!--Numero Diez-->
                       <div class="control-group <?php echo !empty($despacho_servicioError)?'error':'';?>">
					    ...
					    <div class="controls">
					      	<input name="despacho_servicio" autocomplete="off" type="text"  placeholder="# despacho" value="<?php echo !empty($despacho_servicio)?$despacho_servicio:'';?>">
