...
FROM narisol_bladi.customers
    ...
    INNER JOIN narisol_bladi.division_regional 
        ...
        WHERE servicio = 'MENSAJERIA EXPRESA'  AND fecha_entrada = '$fecha'  AND (division_regional.nombre LIKE '%$destino1%') AND customers.destino <> 'CENTRO DE LOS HEROES'
 GROUP BY destino
";
...
$sql="SELECT COUNT(customers.destino) as despacho1 ,SUM(cantidad) AS piezas, COUNT(despacho_servicio) AS despacho, SUM(peso_real) as peso
 FROM narisol_bladi.customers
    ...
    INNER JOIN narisol_bladi.region 
        ON (estafeta.ID_REGION = region.ID_REGION)
    INNER JOIN narisol_bladi.division_regional 
        ON (region.Id_division = division_regional.id_division) WHERE servicio = 'MENSAJERIA EXPRESA'  AND fecha_entrada = '$fecha'  AND (division_regional.nombre LIKE '%$destino1%') AND customers.destino <> 'CENTRO DE LOS HEROES' ";
