<?php 
	...
	if ( null==$id ) {
		header("Location: ../internacional.php");
	}
...
if ($valid) {
        ...
        $sql = "UPDATE customers  set condicion_general = 4 WHERE id = ?";
	    $q = $pdo->prepare($sql);
	    $q->execute(array($id));
		Database::disconnect();
		header("Location: ../internacional.php");
...
<head>
    <meta charset="utf-8">
 <title>INSTITUTO POSRAL DOMINICANO! | BULTO POSTAL</title>
    			...
    			<div class="span10 offset1">
					...
					<!--Comienzo-->
					  ...
					  <div class="control-group <?php echo !empty($destinoError)?'error':'';?>">
					    ...
					    <div class="controls">
					      	<input name="destino" type="text" disabled  value="<?php echo !empty($destino)?$destino:'';?>">
					  ...
					  <!--Numero Diez-->
                       <div class="control-group <?php echo !empty($porqueError)?'error':'';?>">
					    ...
					    <div class="controls">
					      	<input name="porque" type="text"  placeholder="porque" disabled value="<?php echo !empty($porque)?$porque:'';?>">
