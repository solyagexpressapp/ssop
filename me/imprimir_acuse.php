...
$result = mysql_query("SELECT presinto_servicio,despacho_servicio,numero_envio,peso_real, estafeta.destino AS destino,fecha_entrada,pais_origen,destinatario,monitores,oficiales
FROM narisol_bladi.customers
    ...
    INNER JOIN narisol_bladi.division_regional 
        ...
        WHERE  servicio = 'MENSAJERIA EXPRESA'  AND DATE_FORMAT(fecha_entrada,'%Y/%m/%d') = DATE_FORMAT(SYSDATE(),'%Y/%m/%d')  AND  (customers.destino LIKE '%$destino1%') AND  (customers.oficiales LIKE '%$oficial%')
 ORDER BY id desc ");
