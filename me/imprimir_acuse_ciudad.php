...
function redireccionarPagina() {
  window.location = "update_acuse_ciudad.php";
}
...
FROM narisol_bladi.customers
    ...
    INNER JOIN narisol_bladi.division_regional 
        ...
        WHERE  servicio = 'MENSAJERIA EXPRESA' AND division_regional.nombre = 'CIUDAD'  AND oficiales IS NOT NULL 
  AND customers.destino <> 'CENTRO DE LOS HEROES' AND impresion = 1 AND DATE_FORMAT(fecha_entrada,'%Y/%m/%d') = DATE_FORMAT(SYSDATE(),'%Y/%m/%d')
 ORDER BY id desc";
...
/*echo "<td><strong>FECHA DE RECIBO:</strong></td>";*/
...
echo "<td align='center'> <img width='200px'  src='barcodegen/test_1D.php?text=".$row['numero_envio']." ' alt='barcode'> </td>";
echo "</tr>";
echo "<tr>";
echo "<td><strong>CODIGO:</strong></td>";
