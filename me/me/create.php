  ...
  $q = mysql_query("SELECT * FROM customers WHERE numero_envio = '$numero_envio'") or     die(mysql_error());
    if(mysql_num_rows($q) > 0)
   {
    ...
    if ($valid) {
      ...
      $sql = "INSERT INTO customers (servicio,numero_envio,fecha_entrada,destino,destinatario,direccion,peso_real,telefono,descripcion,monitores,oficiales,pais_origen,presinto_servicio,no_recibo) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
      $q = $pdo->prepare($sql);
      $q->execute(array('MENSAJERIA EXPRESA',$numero_envio,$fecha_entrada,$destino,$destinatario,$direccion,$peso_real,$telefono,$descripcion,$monitores,$oficiales,$pais_origen,$presinto_servicio,$recibo+1));
      Database::disconnect();
...
<body>
    <div class="container">
        ...
        <form class="form-horizontal" action="create.php" method="post" onkeypress="return event.keyCode != 13;">
              ...
              <div class="controls">
                  <input name="servicio" type="text"  placeholder="mensajeria" value="MENSAJERIA EXPRESA" disabled>
...
?>
     <!--Segundo-->
            <div class="control-group <?php echo !empty($numero_envioError)?'error':'';?>">
              ...
              <div class="controls">
                  <input name="numero_envio" autocomplete="off" type="text"  placeholder="# de envio"  value="ME<?php echo $id_factura?>DO">
            ...
            <div class="form-actions">
              ...
              <a class="btn" href="../me.php">Atras</a>
            </div>
          </form>
