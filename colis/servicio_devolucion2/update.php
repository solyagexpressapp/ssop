...
if ($valid) {
        ...
        $sql = "UPDATE customers  set presinto_servicio = ?, destino = ?, despacho_servicio = ?, porque= ?, motivo = ? , condicion_general = 1 ,impresion = 1,fecha_entrada = SYSDATE() WHERE id = ?";
	    $q = $pdo->prepare($sql);
	    $q->execute(array($presinto_servicio,$destino,$despacho_servicio,$porque,$devolucion,$id));
		Database::disconnect();
		header("Location: ../devolucion.php");
		}
} else {
