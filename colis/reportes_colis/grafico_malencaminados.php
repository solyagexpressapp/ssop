...
<html>
	<head>
		...
		<title>REPORTE DE MAL ENCAMINADOS POR OFICINA</title>
...
$(function () {
    $('#container').highcharts({
        ...
        title: {
            text: 'REPORTE DE MAL ENCAMINADOS POR OFICINA<'
        },
        subtitle: {
            text: ''
        },
        ...
        yAxis: {
            min: 0,
            title: {
                text: 'Rango de Piezas Mal Encamindas'
            }
        },
        legend: {
            enabled: false
        ...
        tooltip: {
            pointFormat: 'Piezas Mal Encamindas: <b>{point.y:.0f} Piezas</b>'
        },
        series: [{
            name: 'Piezas',
            data: [
               <?php
            $sql=mysql_query(" SELECT servicio,COUNT(DISTINCT(numero_envio)) AS cantidad
FROM devolucion
WHERE servicio = 'COLIS POSTAL' AND motivo = 'MAL ENCAMINADO' and destino = '$ser5[1]' AND fecha_entrada  BETWEEN  '" . $_GET["ser2"]. "' AND  '" . $_GET["ser3"]. "'       
     GROUP BY servicio;
