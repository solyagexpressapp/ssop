<?php 
	...
	if ( !empty($_POST)) {
		...
		if ($valid) {
			...
			$sql = "UPDATE customers  set servicio = ?,numero_envio = ?,destino =?,destinatario = ?,direccion = ?,peso_real = ?,telefono = ?,descripcion = ?,presinto_servicio = ?,despacho_servicio =?,colectora = NULL, despacho_general = NULL ,porque = NULL, motivo = NULL WHERE id = ?";
			$q = $pdo->prepare($sql);
			$q->execute(array('COLIS POSTAL',$numero_envio,$destino,$destinatario,$direccion,$peso_real,$telefono,$descripcion,$presinto_servicio,$despacho_servicio,$id));
			Database::disconnect();
			header("Location: ../servicio_devolucion.php");
		}
	} else {
    ...
    <div class="container">
    			...
    			<div class="span10 offset1">
					...
					<!--Comienzo-->
					    ...
					    <div class="controls">
					      	<input name="servicio" type="text"  placeholder="fecha de entrada" disabled value="COLIS POSTAL">
