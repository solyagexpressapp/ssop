...
FROM narisol_bladi.customers
    ...
    INNER JOIN narisol_bladi.division_regional 
        ON (region.Id_division = division_regional.id_division) 
WHERE fecha_entrada = '2017-08-07' AND estafeta.destino = 'BAVARO' AND colectora IS NOT NULL 
GROUP BY colectora
UNION
...
FROM narisol_bladi.customers
    ...
    INNER JOIN narisol_bladi.division_regional 
        ON (region.Id_division = division_regional.id_division) 
WHERE fecha_entrada = '2017-08-07' AND estafeta.destino = 'BAVARO' AND colectora IS NULL  ";
$result= mysql_query($sql) or die(mysql_error());
...
$hector = mysql_query("SELECT SUM(cantidad) as total from customers where fecha_entrada = '2017-08-08' AND estafeta.destino = 'BAVARO' AND colectora IS NULL  "); 
$row = mysql_fetch_array($hector, MYSQL_ASSOC);
echo $row['total'];
