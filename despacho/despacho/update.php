<?php 
	...
	if ( !empty($_POST)) {
		...
		if ($valid) {
			...
			$sql = "UPDATE customers  set presinto_externo = ?,presinto_servicio = ?,colectora = ?,despacho_general = ? WHERE id = ?";
			$q = $pdo->prepare($sql);
			$q->execute(array($presinto_externo,$presinto_servicio,$colectora,$despacho_general,$id));
			Database::disconnect();
			header("Location: ../despacho.php");
		}
	} else {
		...
		$presinto_externo = $data['presinto_externo'];
    			...
    			<div class="span10 offset1">
					...
					<!--Comienzo-->
                      ...
                      <div class="control-group <?php echo !empty($presinto_externoError)?'error':'';?>">
					    <label class="control-label">Presinto Externo</label>
					    <div class="controls">
					      	<input name="presinto_servicio" type="text" placeholder="Presinto Externo" value="<?php echo !empty($presinto_externo)?$presinto_externo:'';?>">
