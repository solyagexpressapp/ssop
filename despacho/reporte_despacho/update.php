<?php 
	...
	if ( null==$id ) {
		header("Location: ../reporte_despacho.php");
	}
	...
	if ( !empty($_POST)) {
		...
		if ($valid) {
			...
			$sql = "UPDATE customers  set colectora = NULL,despacho_general = ? WHERE id = ?";
			$q = $pdo->prepare($sql);
			$q->execute(array($despacho_general,$id));
			Database::disconnect();
			header("Location: ../reporte_despacho.php");
		}
	} else {
					...
					<!--Comienzo-->
					  ...
					  <!--numero10-->
					   <div class="control-group <?php echo !empty($colectoraError)?'error':'';?>">
					    ...
					    <div class="controls">
					      	<input name="colectora" type="text" placeholder="presinto colector" >
					  ...
					  <div class="form-actions">
						  ...
						  <a class="btn" href="../reporte_despacho.php">Volver</a>
						</div>
					</form>
				</div>
