<?php 
	...
	if ( !empty($_POST)) {
		...
		if ($valid) {
			...
			$q = $pdo->prepare($sql);
			$q->execute(array($servicio,$destino,$descripcion,$presinto_servicio,$despacho_servicio,$id));
			Database::disconnect();
			header("Location: ../corte.php");
		}
	} else {
    			...
    			<div class="span10 offset1">
					...
					<!--Comienzo-->
					    ...
					    <select name="servicio" >
					    	<option><?php echo !empty($servicio)?$servicio:'';?></option>
					    	<option value="CORTE">CORTE</option>
					    	<option value="MENSAJERIA">MENSAJERIA</option>
					    	<option value="ORDINARIO">ORDINARIO</option>
					    	<option value="INPOSPACK">INPOSPACK</option>
					    	<option value="TRANSITO">TRANSITO</option>
					    	<option value="PRECINTOS">PRECINTOS</option>
					    	<option value="SACA ALMACEN">SACA ALMACEN</option>
					 ...
					 <div class="control-group <?php echo !empty($servicioError)?'error':'';?>">
					    <label class="control-label">Servicio</label>
					    <div class="controls">
					      	<input name="servicio" type="text"  placeholder="" value="<?php echo !empty($servicio)?$servicio:'';?>">
