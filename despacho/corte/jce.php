<?php 
	...
	if ( !empty($_POST)) {
        ...
        $presinto_externo = $_POST['presinto_externo'];
...
$sql = "INSERT INTO customers (fecha_entrada,servicio,destino,descripcion,presinto_servicio,despacho_servicio,presinto_externo) values(sysdate(),?,?,?,?,?,?)";
$q = $pdo->prepare($sql);
$q->execute(array('JCE',$destino,$descripcion,$presinto_servicio,$despacho_servicio,$presinto_externo));
Database::disconnect();
header("Location: jce.php");
		}
	}
?>
<!DOCTYPE html>
...
<div class="container" id="izquierda">
    			...
    			<div class="span10 offset1">
    				<div class="row">
		    			<h3>Insertar Despacho de la Junta Central Electoral</h3>
		    		</div>
    		<form class="form-horizontal" action="jce.php" method="post">
	    				<!--Comienzo-->
					  <!--Primero-->
                     ...
                     <!--Octavo-->
					  ...
					  <!--Numero10-->
					     	  <div class="control-group <?php echo !empty($presinto_externoError)?'error':'';?>">
					    <label class="control-label">Presinto del Externo</label>
					    <div class="controls">
					      	<input name="presinto_externo" type="text"  placeholder="Presinto Externo" value="<?php echo !empty($presinto_externo)?$presinto_externo:'';?>">
					      	<?php if (!empty($presinto_externoError)): ?>
					      		<span class="help-inline"><?php echo $presinto_externoError;?></span>
					      	<?php endif; ?>
					    </div>
					  </div>
					  <!--Numero11-->
