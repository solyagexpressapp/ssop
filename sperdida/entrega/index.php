...
<div class="navbar navbar-default navbar-static-top" role="navigation">
    <div class="container">
        ...
        <div class="navbar-header">
            <a class="navbar-brand" href="#" title='Programming Blog'>INSTITUTO POSTAL DOMINICANO | SABANA PERDIDA | Liquidar Piezas a Entregar</a>
        </div>
...
<?php
  $res = $MySQLiconn->query("SELECT id,numero_envio,servicio,fecha_entrada,presinto_servicio,colectora,peso_real,destino,destinatario,telefono
                       from customers WHERE destino IN ('SABANA PERDIDA') AND DATE_FORMAT(fecha_entrada,'%Y/%m/%d') <> DATE_FORMAT(SYSDATE(),'%Y/%m/%d')  and condicion_general = 2 and servicio not in ('ORDINARIO') ORDER BY id DESC");
  $count = $res->num_rows;
