...
FROM narisol_bladi.customers
    ...
    INNER JOIN narisol_bladi.division_regional 
        ...
        WHERE  servicio = 'ORDINARIO'  AND DATE_FORMAT(fecha_entrada,'%Y/%m/%d') = DATE_FORMAT(SYSDATE(),'%Y/%m/%d')  AND  (customers.destino LIKE '%$destino1%') AND  (customers.oficiales LIKE '%$oficial%')
 ORDER BY id desc ");
...
echo "<td align='center'><span class='h'><strong>CORREO ORDINARIO</strong></span><span class='d'><strong>". $row['fecha_entrada'] ."</strong></span></td>";
echo "</tr>";
echo "<tr>";
