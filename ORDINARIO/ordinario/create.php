<?php 
  ...
  $q = mysql_query("SELECT * FROM customers WHERE numero_envio = '$numero_envio'") or     die(mysql_error());
    if(mysql_num_rows($q) > 0)
   {
     ...
     echo '<script>alert("El nombre de envio ya exsite. Por favor elije otro.");</script>';
    ...
    if ($valid) {
      ...
      $sql = "INSERT INTO customers (servicio,numero_envio,fecha_entrada,destino,destinatario,direccion,peso_real,telefono,descripcion,monitores,oficiales,pais_origen) values(?,?,?,?,?,?,?,?,?,?,?,?)";
      $q = $pdo->prepare($sql);
      $q->execute(array('ORDINARIO',$numero_envio,$fecha_entrada,$destino,$destinatario,$direccion,$peso_real,$telefono,$descripcion,$monitores,$oficiales,$pais_origen));
      Database::disconnect();
...
<body>
    <div class="container">
        ...
        <form class="form-horizontal" action="create.php" method="post" onkeypress="return event.keyCode != 13;">
              ...
              <div class="controls">
                  <input name="servicio" type="text"  placeholder="certificado" value="ORDINARIO" disabled>
...
?>
     <!--Segundo-->
            <div class="control-group <?php echo !empty($numero_envioError)?'error':'';?>">
              ...
              <div class="controls">
                  <input name="numero_envio" autocomplete="off" type="text"  placeholder="# de envio"  value="OR<?php echo $id_factura?>DO">
            ...
            <div class="form-actions">
              ...
              <a class="btn" href="../ordinario.php">Atras</a>
            </div>
          </form>
