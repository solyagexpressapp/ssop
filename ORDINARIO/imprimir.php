...
FROM narisol_bladi.customers
    ...
    INNER JOIN narisol_bladi.division_regional 
        ...
        WHERE customers.destino =  '$destino1' AND monitores = '$monitores' AND oficiales = '$oficiales' AND fecha_entrada = '$fecha' AND servicio LIKE '%ORDINARIO%'  
 ORDER BY destino";
...
$sql="SELECT SUM(cantidad) AS piezas, COUNT(despacho_servicio) AS despacho, SUM(peso_real) as peso FROM customers  WHERE destino LIKE  '%$destino1%' AND monitores = '$monitores' AND oficiales = '$oficiales' AND fecha_entrada = '$fecha' AND servicio LIKE '%ORDINARIO%' ";
