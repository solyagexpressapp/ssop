<?php 
	...
	if ( null==$id ) {
		header("Location: ../transito_ordinario.php");
	}
	...
	if ( !empty($_POST)) {
		...
		if ($valid) {
			...
			$sql = "UPDATE customers  set servicio = ?,destino =?,destinatario = ?,direccion = ?,peso_real = ?,telefono = ?,descripcion = ?,presinto_servicio = ?,despacho_servicio =? WHERE id = ?";
			$q = $pdo->prepare($sql);
			$q->execute(array('ORDINARIO',$destino,$destinatario,$direccion,$peso_real,$telefono,$descripcion,$presinto_servicio,$despacho_servicio,$id));
			Database::disconnect();
			header("Location: ../transito_ordinario.php");
		}
	} else {
    			...
    			<div class="span10 offset1">
					...
					<!--Comienzo-->
					  ...
					  <div class="form-actions">
						  ...
						  <a class="btn" href="../transito_ordinario.php">Atras</a>
						</div>
					</form>
				</div>
