<?php 
	...
	if ( null==$id ) {
		header("Location: ../servicio_devolucion.php");
	}
	...
	if ( !empty($_POST)) {
		...
		if ($valid) {
			...
			$sql = "UPDATE customers  set servicio = ?,numero_envio = ?,destino =?,destinatario = ?,direccion = ?,peso_real = ?,telefono = ?,descripcion = ?,presinto_servicio = ?,despacho_servicio =?,colectora = NULL, despacho_general = NULL, porque = NULL, motivo = NULL, fecha_entrada = SYSDATE()  WHERE id = ?";
			$q = $pdo->prepare($sql);
			$q->execute(array(CERTIFICADO,$numero_envio,$destino,$destinatario,$direccion,$peso_real,$telefono,$descripcion,$presinto_servicio,$despacho_servicio,$id));
			Database::disconnect();
			header("Location: ../servicio_devolucion.php");
		}
	} else {
    ...
    <div class="container">
    			...
    			<div class="span10 offset1">
					...
					<!--Comienzo-->
					    ...
					    <div class="controls">
					      	<input name="servicio" type="text"  placeholder="fecha de entrada" disabled value="CERTIFICADO">
					  ...
					  <div class="form-actions">
						  ...
						  <a class="btn" href="../servicio_devolucion.php">Atras</a>
						</div>
					</form>
				</div>
