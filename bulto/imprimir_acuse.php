...
FROM narisol_bladi.customers
    ...
    INNER JOIN narisol_bladi.division_regional 
        ...
        WHERE  servicio = 'BULTO POSTAL'  AND DATE_FORMAT(fecha_entrada,'%Y/%m/%d') = DATE_FORMAT(SYSDATE(),'%Y/%m/%d')  AND  (customers.destino LIKE '%$destino1%') AND  (customers.oficiales LIKE '%$oficial%')
 ORDER BY id desc ");
