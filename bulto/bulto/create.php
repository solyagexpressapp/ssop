...
while ($row = mysql_fetch_array($setencia)) {
 $recibo = $row['LAST'];
  ...
  $q = mysql_query("SELECT * FROM customers WHERE numero_envio = '$numero_envio' AND servicio='BULTO POSTAL'") or     die(mysql_error());
    if(mysql_num_rows($q) > 0)
   {
    ...
    if ($valid) {
      ...
      $q = $pdo->prepare($sql);
      $q->execute(array('BULTO POSTAL',$numero_envio,$fecha_entrada,$destino,$destinatario,$direccion,$peso_real,$telefono,$descripcion,$monitores,$oficiales,$pais_origen,$presinto_servicio,$recibo+1));
      Database::disconnect();
