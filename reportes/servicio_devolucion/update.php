<?php 
	...
	if ( !empty($_POST)) {
		...
		if (empty($porque)) {
		$porque = 'Porfavor introduce un motivo valido';
		$valid = false;
        }
		...
		if ($valid) {
			...
			$sql = "UPDATE customers  set presinto_servicio = ?, despacho_servicio = ?, porque= ?, motivo = ? , condicion_general = 1  WHERE id = ?";
			$q = $pdo->prepare($sql);
			$q->execute(array($presinto_servicio,$despacho_servicio,$porque,$devolucion,$id));
			Database::disconnect();
			header("Location: ../devolucion.php");
		}
	} else {
